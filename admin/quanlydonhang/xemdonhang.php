...
<body>
    ...
    <div id="xemdonhang">
        <?php
            $id=$_GET['id'];
            include('../ketnoi.php');
            $query=mysqli_query($conn,"select * from hoadon where id_hoadon='$id'");
        ...
        <div>
            <table>
                ...
                <?php
                    ...
                    $query=mysqli_query($conn,"select * from chitiet_hoadon where id_hoadon='$id' ");
                    $stt=0;
                    while($row=mysqli_fetch_assoc($query)){
                        $stt++;
                        $idsanpham=$row['id_sp'];
                        $query2=mysqli_query($conn,"select * from sanpham where id='$idsanpham'");
